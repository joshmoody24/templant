...
export const testTemplates = {
  ...
  loopVariables: {
    ...
    // nunjucks: "{% for item in items %}{{ loop.index }}: {{ item }} {% endfor %}",
    // mustache: "{{#items}}{{@index}}: {{ . }} {{/items}}",
  },
  emptyCollections: {
    liquid:
      "{% for item in empty_items %}{{ item }}{% else %}No items{% endfor %}",
    // nunjucks: "{% for item in empty_items %}{{ item }}{% else %}No items{% endfor %}",
  ...
  captures: {
    ...
    // nunjucks: "{% set heading %}Chapter {{ chapter.number }}{% endset %}{{ heading }}",
  ...
  nestedLoops: {
    ...
    // nunjucks: "{% for category in categories %}{% for item in category.items %}{{ category.name }}: {{ item }} {% endfor %}{% endfor %}",
    // mustache: "{{#categories}}{{#items}}{{ ../name }}: {{ . }} {{/items}}{{/categories}}",
  },
  mixedContent: {
    liquid:
      "{% for user in users %}{% if user.active %}{{ user.name | upcase }}{% endif %}{% endfor %}",
    // nunjucks: "{% for user in users %}{% if user.active %}{{ user.name | upper }}{% endif %}{% endfor %}",
  ...
  break: {
    ...
    // nunjucks: "{% for item in items %}{% if item.skip %}{% break %}{% endif %}{{ item }}{% endfor %}",
    // mustache: "{{#items}}{{^skip}}{{ . }}{{/skip}}{{/items}}", // Mustache doesn't have break
  },
  continue: {
    liquid:
      "{% for item in items %}{% if item.skip %}{% continue %}{% endif %}{{ item }}{% endfor %}",
    // nunjucks: "{% for item in items %}{% if item.skip %}{% continue %}{% endif %}{{ item }}{% endfor %}",
  ...
  complexFilters: {
    ...
    // nunjucks: "{{ (products | selectattr('available') | sort(attribute='price') | first) }}",
  ...
  caseStatement: {
    ...
    // nunjucks: "{% if product.type == 'shirt' %}Clothing{% elif product.type == 'book' %}Literature{% else %}Other{% endif %}",
  ...
  liquidObjects: {
    ...
    // mustache: "{{@first}} {{@last}} {{@index}}", // Different loop variables
  },
  emptyOutput: {
    liquid: "{{ '' }}{{ null }}{{ blank }}",
    // nunjucks: "{{ '' }}{{ none }}{{ '' }}",
